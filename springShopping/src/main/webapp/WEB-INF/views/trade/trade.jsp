...
<div id="wrap">
    ...
    <article>
    <form action="/trade/trade" method="get">
	<h2>중고매매</h2>
	...
	<c:choose>
		<c:when test="${pageInfoMap.count gt 0}">
			<c:forEach var="boardVO" items="${boardList}">
				<c:set var="writtenReply" value="${treplyDao.writtenReplyCount(boardVO.num)}" scope="page" />
				<tr class="board_list" onclick="location.href='/trade/tradeContent?num=${boardVO.num}&pageNum=${pageNum}';">
					<td>${boardVO.num}</td>
	...
	<div id="page_control">
		<c:if test="${pageInfoMap.count gt 0}">
			<c:if test="${pageInfoMap.startPage gt pageInfoMap.pageBlock}">
				<a href="/trade/trade?pageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock}&searchType=${searchType}&search=${search}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${pageInfoMap.startPage}" end="${pageInfoMap.endPage}" step="1">
				<a href="/trade/trade?pageNum=${i}&searchType=${searchType}&search=${search}">
				<c:choose>
			...
			<c:if test="${pageInfoMap.endPage lt pageInfoMap.pageCount}">
				<a href="/trade/trade?pageNum=${pageInfoMap.startPage + pageInfoMap.pageBlock}&searchType=${searchType}&search=${search}">[다음]</a>
			</c:if>
		</c:if>
	<div class="clear"></div>
	
		<div class="write_button">
			<input type="button" value="글쓰기" onclick="location.href='/trade/tradeWrite';">
		</div>
	</div>
