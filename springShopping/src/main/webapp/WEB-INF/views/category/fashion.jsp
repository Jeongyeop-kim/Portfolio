...
<div id="wrap">
    ...
    <div id="classify">
    	<div class="classify_box" onclick="location.href='/category/fashion?subc=wom';"><img src="/resources/images_icon/wom.png">여성의류</div>
    	<div class="classify_box" onclick="location.href='/category/fashion?subc=man';"><img src="/resources/images_icon/man.png">남성의류</div>
    	<div class="classify_box" onclick="location.href='/category/fashion?subc=sho';"><img src="/resources/images_icon/sho.png">신발</div> 
	...
	<form name="frm" method="post" action="/category/interest" >
		<div id="interest_view">
		    ...
		    <button type="button" onclick="location.href='/category/interest';">관심항목 이동</button>
			    <div class="align">
				    <a href="/category/fashion?subc=${subc}&sw=readcount">조회수순</a> 
				    | <a href="/category/fashion?subc=${subc}&sw=lowprice">낮은가격순</a>
				    | <a href="/category/fashion?subc=${subc}&sw=highprice">높은가격순</a> 
				    | <a href="/category/fashion?subc=${subc}&sw=date">신상품순</a>
				</div>
		</div>
		<hr>
	...
	<div id="merchandise_wrap">
		<c:choose>
			<c:when test="${pageInfoMap.count gt 0}">
				<c:forEach var="totalVO" items="${fasBoardList}">
					<div id="merchandise">
	...
	<div id="page_control">
		<c:if test="${pageInfoMap.count gt 0}">
			<c:if test="${pageInfoMap.startPage gt pageInfoMap.pageBlock}">
				<a href="/category/fashion?subc=${subc}&sw=${sw}&gPageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock}&gSearch=${gSearch}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${pageInfoMap.startPage}" end="${pageInfoMap.endPage}" step="1">
				<a href="/category/fashion?subc=${subc}&sw=${sw}&gPageNum=${i}&gSearch=${gSearch}">
				<c:choose>
			...
			<c:if test="${pageInfoMap.endPage lt pageInfoMap.pageCount}">
				<a href="/category/fashion?subc=${subc}&sw=${sw}&gPageNum=${pageInfoMap.startPage + pageInfoMap.pageBlock}&gSearch=${gSearch}">[다음]</a>
			</c:if>
		</c:if>
	<div class="clear"></div>
