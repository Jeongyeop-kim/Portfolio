...
<div id="wrap">
    ...
    <div id="classify">
    	<div class="classify_box" onclick="location.href='/category/computer?subc=com';"><img src="/resources/images_icon/com.png">컴퓨터</div>
    	<div class="classify_box" onclick="location.href='/category/computer?subc=ntb';"><img src="/resources/images_icon/ntb.png">노트북</div>
    	<div class="classify_box" onclick="location.href='/category/computer?subc=cpu';"><img src="/resources/images_icon/cpu.png">PC주요부품</div> 
    	<div class="classify_box" onclick="location.href='/category/computer?subc=equ';"><img src="/resources/images_icon/equ.png">PC주변기기</div>
    	<div class="classify_box" onclick="location.href='/category/computer?subc=sto';"><img src="/resources/images_icon/sto.png">PC저장장치</div>
    	<div class="classify_box" onclick="location.href='/category/computer?subc=prt';"><img src="/resources/images_icon/prt.png">프린터</div>
	...
	<form name="frm" method="post" action="/category/interest" >
		<div id="interest_view">
		    ...
		    <button type="button" onclick="location.href='/category/interest';">관심항목 이동</button>
			    <div class="align">
				    <a href="/category/computer?subc=${subc}&sw=readcount">조회수순</a> 
				    | <a href="/category/computer?subc=${subc}&sw=lowprice">낮은가격순</a>
				    | <a href="/category/computer?subc=${subc}&sw=highprice">높은가격순</a> 
				    | <a href="/category/computer?subc=${subc}&sw=date">신상품순</a>
				</div>
		</div>
		<hr>
	...
	<div id="merchandise_wrap">
		<c:choose>
			<c:when test="${pageInfoMap.count gt 0}">
				<c:forEach var="totalVO" items="${comBoardList}">
					<div id="merchandise">
	...
	<div id="page_control">
		<c:if test="${pageInfoMap.count gt 0}">
			<c:if test="${pageInfoMap.startPage gt pageInfoMap.pageBlock}">
				<a href="/category/computer?subc=${subc}&sw=${sw}&gPageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock}&gSearch=${gSearch}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${pageInfoMap.startPage}" end="${pageInfoMap.endPage}" step="1">
				<a href="/category/computer?subc=${subc}&sw=${sw}&gPageNum=${i}&gSearch=${gSearch}">
				<c:choose>
			...
			<c:if test="${pageInfoMap.endPage lt pageInfoMap.pageCount}">
				<a href="/category/computer?subc=${subc}&sw=${sw}&gPageNum=${pageInfoMap.startPage + pageInfoMap.pageBlock}&gSearch=${gSearch}">[다음]</a>
			</c:if>
		</c:if>
	<div class="clear"></div>
