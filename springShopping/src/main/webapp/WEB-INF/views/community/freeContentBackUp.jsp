...
<div id="wrap">
    ...
    <article>
			...
			<div class="attach_box">
				<div id="file_container">
					<c:forEach var="attachVO" items="${attachList}">
						...
						<div id="product">
						<div class="scale"><a href="upload/${attachVO.filename}" download>${attachVO.filename}</a>
							<img src="upload/${attachVO.filename}"></div>
						</div>
						</c:if>
					</c:forEach>
		...
		<c:if test="${replyPageInfoMap.count gt 0}">
			<c:forEach var="replyVO" items="${freplyList}">
				<c:if test="${fboardVO.num eq replyVO.parentnum}">
					...
					<form name="reply${replyVO.num}" action="communityFreeReplyDelete.do" method="post">
					<script>
					var reply = reply${replyVO.num};
					// 댓글 수정
					...
					$(document).on('click', 'span.check${replyVO.num}', function() {
						$('form').attr("action", "communityFreeReplyUpdate.do");
		...
		<div id="reply_pagecontrol">
			<c:if test="${replyPageInfoMap.count gt 0 and fboardVO.num eq parNum}">
				<c:if test="${replyPageInfoMap.startPage gt replyPageInfoMap.pageBlock}">
					<a href="communityFreeContent.do?replyPageNum=${replyPageInfoMap.startPage - replyPageInfoMap.pageBlock}&num=${fboardVO.num}&pageNum=${pageNum}">[이전]</a>
				</c:if>
				
				<c:forEach var="i" begin="${replyPageInfoMap.startPage}" end="${replyPageInfoMap.endPage}" step="1">
					<a href="communityFreeContent.do?replyPageNum=${i}&num=${fboardVO.num}&pageNum=${pageNum}">
					<c:choose>
				...
				<c:if test="${replyPageInfoMap.endPage lt replyPageInfoMap.pageCount}">
					<a href="communityFreeContent.do?replyPageNum=${replyPageInfoMap.startPage + replyPageInfoMap.pageBlock}&num=${fboardVO.num}&pageNum=${pageNum}">[다음]</a>
				</c:if>
			</c:if>
		</div>
		...
		<form name="frm" action="communityFreeReply.do" method="post" onsubmit="return replyCheck();">
		...
		<c:if test="${not empty loginMember}">
			<c:if test="${loginMember.id eq fboardVO.username or loginMember.id eq 'admin'}">
				<input type="submit" value="수정하기" class="btn" onclick="location.href='communityFreeUpdateForm.do?num=${fboardVO.num}&pageNum=${pageNum}';">
				<input type="submit" value="삭제하기" class="btn" onclick="location.href='communityFreeDelete.do?num=${fboardVO.num}&pageNum=${pageNum}';">
			</c:if>
		<input type="submit" value="답글쓰기" class="btn" onclick="location.href='communityFreeReWriteForm.do?reRef=${fboardVO.reRef}&reLev=${fboardVO.reLev}&reSeq=${fboardVO.reSeq}';">
		</c:if>
		<input type="submit" value="목록보기" class="btn" onclick="location.href='communityFree.do?pageNum=${pageNum}';">
		</div>
    </article>
...
$('#suggest').on('click', function() {
	...
	$.ajax({
		url: 'communityGoodJson.do',
		data: {username: username, pageNum: pageNum, num: num, good: good},
