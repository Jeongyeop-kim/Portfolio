...
// 					});
					...
					// 대댓글
					$(document).one('click', 'span.add${replyVO.num}', function() {
						var $add = $('span.add${replyVO.num}').closest('form');
						$add.children('div#reply').append('<br><div id="reply_content" style="display: block; margin-top: 20px; margin-bottom: 20px;"><textarea class="inputReply" name="content" rows="6" cols="123"></textarea><input type="submit" value="댓글쓰기" class="btn"></div><br>');
					});
					</script>
					<div id="reply">
						<div class="reply_id">${replyVO.username}
						<c:if test="${not empty loginMember and loginMember.id eq replyVO.username}">
							<span class="u${replyVO.num}">수정</span> | <span class="d${replyVO.num}">삭제</span> | 
						</c:if>
						<span class="add${replyVO.num}">댓글달기</span><br>
						</div>
