...
<div id="wrap">
    ...
    <article>
    <form action="/community/free" method="get">
	<h2>자유 게시판</h2>
	...
	<div id="page_control">
		<c:if test="${freePageInfoMap.count gt 0}">
			<c:if test="${freePageInfoMap.startPage gt freePageInfoMap.pageBlock}">
				<a href="/community/free?pageNum=${freePageInfoMap.startPage - freePageInfoMap.pageBlock}&searchType=${searchType}&search=${search}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${freePageInfoMap.startPage}" end="${freePageInfoMap.endPage}" step="1">
				<a href="/community/free?pageNum=${i}&searchType=${searchType}&search=${search}">
				<c:choose>
			...
			<c:if test="${freePageInfoMap.endPage lt freePageInfoMap.pageCount}">
				<a href="/community/free?pageNum=${freePageInfoMap.startPage + freePageInfoMap.pageBlock}&searchType=${searchType}&search=${search}">[다음]</a>
			</c:if>
		</c:if>
	<div class="clear"></div>
	
		<div class="write_button">
			<input type="button" value="글쓰기" onclick="location.href='/community/freeWrite';">
		</div>
	</div>
