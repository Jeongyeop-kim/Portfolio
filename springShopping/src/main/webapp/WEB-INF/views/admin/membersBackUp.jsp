...
<div id="wrap">
	...
	<div id="member_list_wrap">
		<div class="buttons">
			<form name="frm" action="adminMembers.do" method="get">
				<input type="text" name="mSearch" value="${empty mSearch ? '' : mSearch}" class="mSearch">
				<input type="submit" value="검색">
			</form>
		</div>
		<form name="dfrm" action="deleteMembers.do" method="get">
			...
			<c:if test="${membersPageInfoMap.count gt 0}">
				<c:forEach var="memberVO" items="${mBoardList}">
					<div class="member_list_content">
						<div class="member_list_checkbox"><input type="checkbox" name="check" value="${memberVO.id}"></div>
						<div class="member_list_id">${memberVO.id}</div>
						<div class="member_list_passwd">${memberVO.passwd}</div>
	...
	<div id="page_control">
		<c:if test="${membersPageInfoMap.count gt 0}">
			<c:if test="${membersPageInfoMap.startPage gt membersPageInfoMap.pageBlock}">
				<a href="adminMembers.do?mPageNum=${membersPageInfoMap.startPage - membersPageInfoMap.pageBlock}&mSearch=${mSearch}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${membersPageInfoMap.startPage}" end="${membersPageInfoMap.endPage}" step="1">
				<a href="adminMembers.do?mPageNum=${i}&mSearch=${mSearch}">
				<c:choose>
			...
			<c:if test="${membersPageInfoMap.endPage lt membersPageInfoMap.pageCount}">
				<a href="adminMembers.do?mPageNum=${membersPageInfoMap.startPage + membersPageInfoMap.pageBlock}&mSearch=${mSearch}">[다음]</a>
			</c:if>
		</c:if>
	</div>
